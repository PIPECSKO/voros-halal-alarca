...
game.on('player-list-updated', (players) => {
  ui.updatePlayerList(players);
});

game.on('role-assigned', (roleInfo) => {
  ui.updateRoleInfo(roleInfo);
});

game.on('tasks-assigned', (tasks) => {
  ui.updateTaskList(tasks);
});

game.on('game-message', (message) => {
  ui.showGameMessage(message);
});

game.on('timer-update', (time) => {
  ui.updateGameTimer(time);
});

game.on('game-ended', (data) => {
  ui.showGameEnd(data.winner, data.message, data.roles);
});
